...
class FlowStatDb:
    ...
    def connect(self):
        ...
        _logger.debug("Opened database %s successfully" % self.dbpath)
    ...
    def _executemany(self, cur, sql, params):
        _logger.debug(sql)
        cur.executemany(sql, params)

    def _fetchone(self, cur, sql):
        self._execute(cur, sql)
        return cur.fetchone()

    def _fetchall(self, cur, sql):
        self._execute(cur, sql)
        return cur.fetchall()
    ...
    def _upsert(self, table, primary_keys, cols, values, cur=None):
        ...
        '''
        self._executemany(c, sql, values)
        ...
        while need_reading and index >= 0:
            ...
            with open(curfile, 'r') as f:
                ...
                while True:
                    ...
                    # only break if moved to next period
                    if line_number >= limit:
                        if changed_ts:
                            _logger.debug(f'Read limited line:{line_number}')
                            need_reading = False
                            break
                        elif line_number % limit == 0:
                            # still not moved to next period, let take a rest
                            time.sleep(self.read_limit_interval)
    ...
    def prepare_temp_data(self, data):
        ...
        for d in data:
            ...
            if len(to_inserts) >= self.temp_insert_batch:
                ...
                self._executemany(c, sql, data_insert)
                _logger.debug('Inserted %d items' % len(to_inserts))
                to_inserts = []
        ...
        for table in AGG_TABLE_LIST:
            for period in PERIOD_LIST:
                ...
                sql = f'''
                    ...
                    limit 1;
                '''
                last_time = self._fetchone(c, sql)
                if last_time:
                ...
                records = self._fetchall(c, sql)
                _logger.debug(f'Found new {tbname} {len(records)} items')
                ...
                _logger.debug(f'Updated {tbname} {c.rowcount} items')
        c.close()
    ...
    def _load_metadata(self):
        ...
        sql = f'''
            ...
            from metadata
        '''
        rec = self._fetchone(c, sql)
        _logger.debug(f'read lastlog: {rec}')
        if rec:
    ...
    def _save_metadata(self):
        _logger.debug(f'save lastlog: {self.last_log}')
        c = self.conn.cursor()
        upsert_cols = [
            'id',
            'data',
    ...
    def query_time_series(self, alias, start_time, end_time, if_name=None,
        ...
        if filter_key:
            table_key = table['key']
            where += f' and {table_key} = "{filter_key}"'
    ...
    def vacuum(self):
        ...
        for table in AGG_TABLE_LIST:
            for period in PERIOD_LIST:
                ...
                self._execute(c, sql)
                if c.rowcount:
                    _logger.debug(f'Deleted from {table["name"]}_{period} {c.rowcount} items')
    ...
    def show_info(self):
        ...
        for table in AGG_TABLE_LIST:
            for period in PERIOD_LIST:
                ...
                count = self._fetchone(c, sql)
                if count:
                    count = count[0]
                else:
                    count = 0

                _logger.debug(f'{table_name:<20} ({count:08}) {start:>10} => {end:>10}')
...
def collect_statistics(db_file, log_file):
    ...
    _logger.info('Collect finished in %.2fs' % (time.time() - st))
